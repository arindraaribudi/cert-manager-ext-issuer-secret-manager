// Command demo-aws proves the controller's fetch + parse + secret-build pipeline
// end-to-end against the fake. ponytail: non-trivial logic leaves one runnable
// check; this is it for the AWS path.
package main
...
func run() error {
	...
	fc.Set("demo-secret", payload)

	got, err := fc.Fetch(ctx, "demo-secret")
	if err != nil {
	...
	fmt.Printf("aws: built Secret %s/%s with %d cert bytes, %d key bytes, %d chain bytes\n",
