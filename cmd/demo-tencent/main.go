// Command demo-tencent proves the controller's fetch + parse + secret-build pipeline
// end-to-end against the fake. ponytail: non-trivial logic leaves one runnable
// check; this is it for the Tencent path.
package main
...
func run() error {
	...
	fc.Set("abc123", payload)

	got, err := fc.Fetch(ctx, "abc123")
	if err != nil {
	...
	fmt.Printf("tencent: built Secret %s/%s with %d cert bytes, %d key bytes, %d chain bytes\n",
